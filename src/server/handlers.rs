...
<head>
    ...
    <style>
        :root{
            ...
            --success:#404040;--success-bg:#f0f0f0;--success-fg:#262626;
            --danger:#121212;--danger-bg:#e8e8e8;--danger-fg:#121212;
            --warning:#6b6b6b;--warning-bg:#f0f0f0;--warning-fg:#595959;
        ...
        :root[data-theme="dark"]{
            ...
            --success:#404040;--success-bg:#1e1e1e;--success-fg:#b0b0b0;
            --danger:#ffffff;--danger-bg:#262626;--danger-fg:#ffffff;
            --warning:#6b6b6b;--warning-bg:#1e1e1e;--warning-fg:#b0b0b0;
        ...
        *{margin:0;padding:0;box-sizing:border-box;font-family:system-ui,-apple-system,sans-serif}
        ...
        .sidebar-subgroup{display:none;padding-left:8px;margin-bottom:2px}
        .sidebar-item.active+.sidebar-subgroup{display:block}
        .sidebar:not(.expanded) .sidebar-subgroup{display:none!important}
        .sidebar-subitem{display:flex;align-items:center;gap:8px;min-height:32px;padding:6px 8px 6px 26px;border-radius:var(--radius);border:none;background:none;color:var(--dim);font-size:13px;font-weight:500;cursor:pointer;width:100%;white-space:normal;text-align:left;line-height:1.3;transition:background .12s,color .12s;position:relative}
        .sidebar-subitem::before{content:'';position:absolute;left:14px;top:16px;width:6px;height:1px;background:var(--border)}
        .sidebar-subitem:hover{background:var(--muted);color:var(--fg)}
        .sidebar-subitem.active{background:var(--muted);color:var(--fg);font-weight:700;box-shadow:inset 2px 0 0 var(--fg)}
        ...
        .toast-ok{background:#404040;color:#fff}.toast-err{background:#121212;color:#fff}
        ...
        .stat-info{color:var(--fg)}.stat-ok{color:#404040}.stat-warn{color:#6b6b6b}.stat-err{color:#121212}
        ...
        .qp-overall.green{background:#404040}.qp-overall.yellow{background:#6b6b6b}.qp-overall.red{background:#121212}
        ...
        .qp-sub.green{background:#404040}.qp-sub.yellow{background:#6b6b6b}.qp-sub.red{background:#121212}
        ...
        .qp-sev-c{color:#121212}.qp-sev-w{color:#6b6b6b}.qp-sev-i{color:#767676}
        ...
        /* OOD banner */
        .ood-banner{background:#40404020;border:1px solid #6b6b6b;border-radius:var(--radius);padding:10px 14px;display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:10px}
        .ood-banner-text{font-size:13px;color:#6b6b6b;line-height:1.5}
...
<body>
    <div class="app">
        <aside class="sidebar" id="e-sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <div class="sidebar-subgroup" data-parent="train">
                    <button class="sidebar-subitem active" data-parent="train" data-name="autotune" onclick="eTab('train');eSubTab('train','autotune')"><span>Auto-Tune</span></button>
                    <button class="sidebar-subitem" data-parent="train" data-name="automl" onclick="eTab('train');eSubTab('train','automl')"><span>AutoML</span></button>
                    <button class="sidebar-subitem" data-parent="train" data-name="single" onclick="eTab('train');eSubTab('train','single')"><span>Single Model</span></button>
                    <button class="sidebar-subitem" data-parent="train" data-name="hyperopt" onclick="eTab('train');eSubTab('train','hyperopt')"><span>HyperOpt</span></button>
                    <button class="sidebar-subitem" data-parent="train" data-name="ensemble" onclick="eTab('train');eSubTab('train','ensemble')"><span>Ensemble</span></button>
                    <button class="sidebar-subitem" data-parent="train" data-name="history" onclick="eTab('train');eSubTab('train','history');eHistLoad()"><span>History</span></button>
                </div>
                ...
                <div class="sidebar-subgroup" data-parent="analysis">
                    <button class="sidebar-subitem active" data-parent="analysis" data-name="explain" onclick="eTab('analysis');eSubTab('analysis','explain')"><span>Explainability</span></button>
                    <button class="sidebar-subitem" data-parent="analysis" data-name="anomaly" onclick="eTab('analysis');eSubTab('analysis','anomaly')"><span>Anomaly</span></button>
                    <button class="sidebar-subitem" data-parent="analysis" data-name="dimred" onclick="eTab('analysis');eSubTab('analysis','dimred')"><span>UMAP / PCA</span></button>
                </div>
                <button class="sidebar-item" data-tab="insights" onclick="eTab('insights')"><i class="ri-lightbulb-line"></i><span>Insights</span></button>
                <div class="sidebar-subgroup" data-parent="insights">
                    <button class="sidebar-subitem active" data-parent="insights" data-name="stepthrough" onclick="eTab('insights');eSubTab('insights','stepthrough');eInsLoad()"><span>How It Learned</span></button>
                    <button class="sidebar-subitem" data-parent="insights" data-name="concepts" onclick="eTab('insights');eSubTab('insights','concepts')"><span>Understand the Model</span></button>
                    <button class="sidebar-subitem" data-parent="insights" data-name="playground" onclick="eTab('insights');eSubTab('insights','playground');eInsPgLoad()"><span>What If?</span></button>
                    <button class="sidebar-subitem" data-parent="insights" data-name="metrics" onclick="eTab('insights');eSubTab('insights','metrics');eInsMetricsLoad()"><span>How Well Did It Do?</span></button>
                </div>
        ...
        <div class="main-wrap">
            <div class="topbar">
                ...
                <div class="topbar-meta">
                    ...
                    <span id="e-dot" style="width:8px;height:8px;border-radius:50%;background:#404040;display:inline-block"></span>
                </div>
            </div>
            <div class="content">
                        ...
                        <div class="grid-2">
                            ...
                            <div class="card">
                                ...
                                <div style="display:flex;flex-direction:column;gap:12px">
                                    ...
                                    <div><div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:4px"><span>Memory</span><span class="mono" id="e-dash-mem2">0%</span></div><div class="progress"><div class="progress-fill" id="e-dash-mem-bar" style="width:0%;background:#404040"></div></div></div>
                    ...
                    <div id="et-monitor" class="tab-panel">
                        ...
                        <div class="grid-2">
                            ...
                            <div class="card">
                                ...
                                <div class="progress" style="height:10px"><div class="progress-fill" id="e-mon-mem-bar" style="width:0%;height:100%;background:#404040"></div></div>
    ...
    function eHo3dChart(trials,bestTrial){
        ...
        if(paramKeys.length===1){
            ...
            var bestTrace={
                ...
                marker:{symbol:'star',size:16,color:'#6b6b6b',line:{color:'#ffffff',width:1}},
                text:['BEST'],textposition:'top right',textfont:{size:10,color:'#6b6b6b'},
            ...
            var bestTrace={
                ...
                marker:{symbol:'diamond',size:8,color:'#6b6b6b',line:{color:'#ffffff',width:1}},
                text:['BEST'],textposition:'top right',textfont:{size:10,color:'#6b6b6b'},
                customdata:[best.trial_id],
    ...
    function eDrawBars(canvasId,items,opts){
        ...
        var colors=opts.colors||['#000000','#444444','#666666','#888888','#aaaaaa','#cccccc','#111111','#333333','#777777','#999999','#bbbbbb','#dddddd','#eeeeee','#050505'];
    ...
    function eDrawScatter(canvasId,points,opts){
        ...
        function tx(v){return pad.l+(v-xMin)/xRange*pW}
        function ty(v){return pad.t+pH-(v-yMin)/yRange*pH}
        var colors=opts.colors||['#000000','#444444','#666666','#888888','#cccccc','#aaaaaa','#050505','#111111'];
    ...
    function eDrawDonut(canvasId,segments,opts){
        ...
        var colors=opts.colors||['#000000','#444444','#666666','#888888','#cccccc','#050505'];
        var frame=0,dur=50;
        function ease(t){return 1-Math.pow(1-t,3)}
        function draw(){
            ctx.clearRect(0,0,W,H);
    ...
    function eDrawMonitorSpark(){
        ...
        if(eMemHistory.length>1)eDrawSparkline('e-spark-mem',eMemHistory,'#404040');
    }
    ...
    function eRunAnalysis(){
        ...
        fetch('/api/data/analyze').then(function(r){return r.json()}).then(function(d){
            ...
            var qcol=qpct>=80?'#404040':qpct>=60?'#6b6b6b':'#121212';
            ...
            oh+='<div style="text-align:center;padding:14px;background:#f0f0f0;border-radius:0"><span class="stat stat-sm stat-ok stat-animated">'+ov.columns+'</span><p style="font-size:11px;color:#404040;margin-top:2px">Columns</p></div>';
            oh+='<div style="text-align:center;padding:14px;background:'+(ov.duplicates>0?'#f0f0f0':'#f0f0f0')+';border-radius:0"><span class="stat stat-sm '+(ov.duplicates>0?'stat-warn':'stat-ok')+' stat-animated">'+ov.duplicates+'</span><p style="font-size:11px;color:#404040;margin-top:2px">Duplicates</p></div>';
            ...
            // Quality gauges
            var dims=[{l:'Completeness',v:q.completeness,c:'#121212'},{l:'Uniqueness',v:q.uniqueness,c:'#121212'},{l:'Consistency',v:q.consistency,c:'#404040'},{l:'Validity',v:q.validity,c:'#121212'}];
            ...
            if(cs.length>0){
                ...
                cs.forEach(function(c){
                    ...
                    var missBg=missPct>10?'#e8e8e8':missPct>0?'#f0f0f0':'transparent';
                    var skewColor=c.skewness!=null&&Math.abs(c.skewness)>2?'#6b6b6b':'inherit';
                    var kurtColor=c.kurtosis!=null&&Math.abs(c.kurtosis)>7?'#6b6b6b':'inherit';
                    var outColor=(c.outlier_percent||0)>10?'#121212':(c.outlier_percent||0)>5?'#6b6b6b':'inherit';
                    var zeroColor=(c.zeros_percent||0)>50?'#6b6b6b':'inherit';
                    th+='<tr>';
            ...
            if(outWithData.length>0){$('e-outlier-section').style.display='block';var ot='<div style="display:flex;flex-wrap:wrap;gap:10px">';outWithData.forEach(function(o){var pct=o.outlier_percent||0;var col=pct>5?'#121212':pct>1?'#6b6b6b':'#121212';ot+='<div style="padding:12px 16px;background:#f8f8f8;border-radius:0;border-left:3px solid '+col+';min-width:180px"><div style="font-weight:500;font-size:13px;margin-bottom:4px">'+o.column+'</div><div style="font-size:22px;font-weight:600;color:'+col+';font-family:\'Geist Mono\',monospace">'+o.outlier_count+'</div><div style="font-size:11px;color:#404040">'+pct.toFixed(1)+'% outliers</div><div class="mono" style="font-size:10px;color:#767676;margin-top:4px">bounds: ['+o.lower_bound.toFixed(1)+', '+o.upper_bound.toFixed(1)+']</div></div>'});ot+='</div>';$('e-outlier-table').innerHTML=ot}else{$('e-outlier-section').style.display='none'}
            // Missing values chart
            var mv=d.missing_values||[];
            var mvWithData=mv.filter(function(m){return m.null_count>0});
            if(mvWithData.length>0){$('e-missing-section').style.display='block';mvWithData.sort(function(a,b){return b.null_percent-a.null_percent});var mh='';mvWithData.forEach(function(m){var pct=m.null_percent||0;var col=pct>50?'#121212':pct>10?'#6b6b6b':'#121212';mh+='<div style="display:flex;align-items:center;gap:12px;margin-bottom:8px"><span style="font-size:12px;font-weight:500;width:140px;text-align:right;flex-shrink:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+m.column+'</span><div style="flex:1;height:20px;background:#f0f0f0;border-radius:0;overflow:hidden;position:relative"><div style="height:100%;width:'+Math.max(pct,0.5)+'%;background:'+col+';border-radius:0;transition:width .3s"></div></div><span class="mono" style="font-size:12px;width:60px;flex-shrink:0;color:'+col+'">'+pct.toFixed(1)+'%</span></div>'});$('e-missing-chart').innerHTML=mh}else{$('e-missing-section').style.display='none';$('e-missing-chart').innerHTML='<p style="font-size:13px;color:#404040"><i class="ri-check-line"></i> No missing values detected</p>'}
            eLogActivity('Data analysis complete: '+qpct+'% quality score');
        }).catch(function(e){$('e-analysis-loading').style.display='none';$('e-analysis-content').style.display='block';$('e-analysis-content').innerHTML='<p style="color:#121212;font-size:13px">Analysis failed: '+e.message+'</p>'})
    }
    ...
    function eRunDimRed(){
        ...
        fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){
            ...
            if(d.success&&d.points){
                ...
                var pts=[];var colors=['#121212','#404040','#6b6b6b','#121212','#121212','#121212','#121212','#121212'];
                if(d.color_mode==='classification'&&d.labels){
                    ...
                    $('e-dr-legend').innerHTML='<div class="chart-legend-item"><div class="chart-legend-dot" style="background:#121212"></div>Low</div><div class="chart-legend-item"><div class="chart-legend-dot" style="background:#121212"></div>High</div>';
                }else{
                ...
                var infoHtml='<div class="grid-2"><div style="text-align:center;padding:8px;background:#f0f0f0;border-radius:0"><span class="stat stat-sm stat-info">'+d.n_samples+'</span><p style="font-size:11px;color:#404040">Samples</p></div><div style="text-align:center;padding:8px;background:#f0f0f0;border-radius:0"><span class="stat stat-sm stat-ok">'+method.toUpperCase()+'</span><p style="font-size:11px;color:#404040">Method</p></div></div>';
                ...
                if(d.explained_variance_ratio){
                    ...
                    d.explained_variance_ratio.forEach(function(v,i){
                        ...
                        vh+='<div style="margin-bottom:6px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:2px"><span>PC'+(i+1)+'</span><span class="mono">'+pct+'%</span></div><div class="progress"><div class="progress-fill" style="width:'+pct+'%;background:'+(i===0?'#121212':i===1?'#404040':'#6b6b6b')+'"></div></div></div>'
                    });
    ...
    function eInsStDrawConvergence() {
      ...
      if (hasVal) {
        ctx.strokeStyle = '#6b6b6b'; ctx.lineWidth = 2; ctx.setLineDash([4,4]);
        ctx.beginPath();
        epochs.forEach(function(r, i) {
    ...
    function eInsConceptsDrawBV(epochHistory) {
      ...
      if (hasValAcc) {
        ctx.fillStyle = '#6b6b6b'; ctx.fillRect(pad + 60, pad + 4, 12, 3);
    ...
    function eInsConceptsDrawSplit() {
      ...
      ctx.fillStyle = '#6b6b6b'; ctx.fillRect(pad.left + trainW, pad.top, valW, barH);
      ctx.fillStyle = '#404040'; ctx.fillRect(pad.left + trainW + valW, pad.top, testW, barH);
    ...
    function eInsPgDrawAttribution(d) {
      ...
      sorted.forEach(function(attr, i) {
        ...
        // Arrow line
        ctx.strokeStyle = val >= 0 ? '#404040' : '#121212';
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(cx, y);
        ...
        // Arrowhead
        var dir = val >= 0 ? 1 : -1;
        ctx.fillStyle = val >= 0 ? '#404040' : '#121212';
        ctx.beginPath();
        ctx.moveTo(endX, y);
  ...
  counts.forEach(function(c, i) {
    ...
    ctx.fillStyle = binMid < 0 ? '#121212' : '#404040';
  ...
  ctx.fillText('Residual (Actual - Predicted)', W/2, H - pad + 4);
    ...
    function eQualityPreCard(pt,score){
      ...
      if(leakage.length>0){h+='<div class="qp-label">Leakage suspects</div><div>'+leakage.map(function(f){return'<span class="qp-tag" style="background:#2a2a2a;color:#121212">'+escHtml(f)+'</span>'}).join('')+'</div>'}
    ...
    function eQualityTrainCard(tr,score){
      ...
      if(excl.length>0){h+='<div class="qp-label">Algorithms excluded</div>';excl.forEach(function(e){h+='<div class="qp-row"><span style="color:#6b6b6b"><i class="ri-alarm-warning-line"></i></span><span>'+escHtml(String(e).replace(/^EXCLUDED\s+/,''))+'</span></div>'})}
      var ensResult=(tr&&tr.ensemble_beat_single)?'<span style="color:#404040"><i class="ri-check-line"></i> Ensemble beat single model</span>':'<span style="color:#767676">Ensemble did not improve on best single model</span>';
    ...
    function eQualityHyperoptCard(ho,score,safeId){
      ...
      h+='<div class="qp-row">'+(ho&&ho.converged?'<span style="color:#404040"><i class="ri-check-line"></i> Converged</span>':'<span style="color:#6b6b6b"><i class="ri-alarm-warning-line"></i> Did not converge</span>')+'</div>';
    ...
    function eQualityPostCard(po,score){
      ...
      if(po&&po.ece_before!=null&&po.ece_after!=null){h+='<div class="qp-row"><span class="qp-label" style="margin:0;margin-right:6px">ECE</span><span>'+po.ece_before.toFixed(3)+' &#8594; <span style="color:#404040">'+po.ece_after.toFixed(3)+'</span></span></div>'}
